...
## Train set:
set.seed(123);Building_Model_ranger <- ranger(BUILDINGID ~., data = trainingData[, c(WAPs, "BUILDINGID")])
...
## Train set:
set.seed(123);BF_Model_ranger <- ranger(BUILDINGID_FLOOR ~., data = trainingData[, c(WAPs, "BUILDINGID_FLOOR")]) 
...
## Train set:
set.seed(123);Building_ModelTop3_ranger <- ranger(BUILDINGID ~., data = trainingData[, c(predictors_train)])
...
## Train set:
set.seed(123);BF_Model_allwaps <- ranger(BUILDINGID_FLOOR ~.,trainingData[, c(WAPs, "BUILDINGID_FLOOR")])
...
#### the highest signal is captured
## Train set:
set.seed(123);BF_ModelTop3_ranger <- ranger(BUILDINGID_FLOOR ~., trainingData[, c("TopWap1", "TopWap2", "TopWap3", "BUILDINGID_FLOOR")])
...
## Train set:
set.seed(123);Latitude_Model <- ranger(LATITUDE ~., data = trainingData[, c(WAPs, "LATITUDE")])
...
## plotting errors
pabserror_split(Latitude_Model_pred_val$predictions, validationData$LATITUDE, validationData$BUILDINGID, "LATITUDE")
...
## Train set:
set.seed(123);Longitude_Model <- ranger(LONGITUDE ~., data = trainingData[, c(WAPs, "LONGITUDE")])
...
## plotting errors
pabserror_split(Longitude_Model_pred_val$predictions, validationData$LONGITUDE, validationData$BUILDINGID, "LONGITUDE")
